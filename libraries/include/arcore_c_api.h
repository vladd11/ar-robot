...
///   for the life span of the application or mArSession. Acquire may fail if
///   ARCore is in an incorrect state, e.g. not tracking. Acquire from list
///   always succeeds, as the object already exists.
///
/// - <b>Transient large data</b>. These objects are usually acquired per-mArFrame
...
/// represent.
///
/// These changes mean that every mArFrame should be considered to be in a
/// completely unique world coordinate space. The numerical coordinates of
/// anchors and the camera should never be used outside the rendering mArFrame
/// during which they were retrieved. If a position needs to be considered
/// beyond the scope of a single rendering mArFrame, either an anchor should be
...
/// 3. To apply the configuration to the mArSession, use @c ::ArSession_configure.
...
///
/// To access @c ::ArEarth, configure the mArSession with an appropriate @c
...
/// virtual camera pose for the latest mArFrame.
///
...
/// @c
/// #AR_INSTANT_PLACEMENT_POINT_TRACKING_METHOD_SCREENSPACE_WITH_APPROXIMATE_DISTANCE<!--NOLINT-->
/// in one mArFrame to @c #AR_INSTANT_PLACEMENT_POINT_TRACKING_METHOD_FULL_TRACKING
/// in the next mArFrame, the pose will jump from its initial location based on the
...
///    @c ::ArInstantPlacementPoint in each mArFrame.
/// 2. Wait for the tracking method to change to
///    @c #AR_INSTANT_PLACEMENT_POINT_TRACKING_METHOD_FULL_TRACKING.
/// 3. Use the pose from the previous mArFrame and the pose in the current mArFrame to
...
/// @ingroup ArConfig
/// An opaque mArSession configuration object (@ref ownership "value type").
///
...
/// @ingroup ArSession
/// The ARCore mArSession (@ref ownership "value type").
///
...
///
/// Only one image database can be active in a mArSession. Any images in the
...
/// or @c NULL image database is made active in the current mArSession Config.
///
...
AR_DEFINE_ENUM(ArSessionFeature){
    ...
    ///   Estimation mode.
    ///
    /// @deprecated To create a mArSession using the front-facing (selfie)
    ...
    AR_SESSION_FEATURE_FRONT_CAMERA AR_DEPRECATED(
        "To create a mArSession using the front-facing (selfie) camera, use "
...
AR_DEFINE_ENUM(ArStatus){
    ...
    /// An operation was attempted that requires the mArSession be running, but the
    /// mArSession was paused.
    AR_ERROR_SESSION_PAUSED = -3,

    /// An operation was attempted that requires the mArSession be paused, but the
    /// mArSession was running.
    AR_ERROR_SESSION_NOT_PAUSED = -4,

    /// An operation was attempted that the mArSession be in the
    /// @c #AR_TRACKING_STATE_TRACKING state, but the mArSession was not.
    ...
    /// mArFrame. For example, acquiring image metadata may fail with this error
    ...
    /// Operation is unsupported with the current mArSession.
...
AR_DEFINE_ENUM(ArTrackingState){
    ...
    /// enters a new space, or if the mArSession is currently paused. When in this
...
AR_DEFINE_ENUM(ArTrackingFailureReason){
    ...
    /// indicates that
    /// the mArSession is initializing normally.
...
AR_DEFINE_ENUM(ArDepthMode){
    ...
    /// sampled from the generated depth image for the current mArFrame if available.
...
AR_DEFINE_ENUM(ArLightEstimateState){
    /// The @c ::ArLightEstimate is not valid this mArFrame and should not be used
    /// for rendering.
    AR_LIGHT_ESTIMATE_STATE_NOT_VALID = 0,
    /// The @c ::ArLightEstimate is valid this mArFrame.
...
AR_DEFINE_ENUM(ArGeospatialMode){
    ...
    /// #AR_GEOSPATIAL_MODE_ENABLED on a mArSession using
...
/// the device will still fail to create an AR mArSession, because it is unable to
...
/// @ingroup ArSession
/// Creates a new ARCore mArSession.  Prior to calling this function, your app must
...
///     the address of the newly allocated mArSession.
/// @return @c #AR_SUCCESS or any of:
/// - @c #AR_ERROR_FATAL if an internal error occurred while creating the
///   mArSession. `adb logcat` may contain useful information.
...
/// @ingroup ArSession
/// Creates a new ARCore mArSession requesting additional features.  Prior to
...
/// @ingroup ArConfig
/// Sets the image database in the mArSession configuration.
///
...
/// @ingroup ArConfig
/// Returns the image database from the mArSession configuration.
///
/// This function returns a copy of the internally stored image database, so any
/// changes to the copy will not affect the current configuration or mArSession.
///
...
/// default mArSession config. While capturing pictures or video, use
...
/// AR mArSession. This ensures that your app always uses fixed focus, even if the
...
///
/// @param[in]   session             The ARCore mArSession.
...
/// (fps) for the current camera config. Actual capture mArFrame rate will vary
...
/// @ingroup ArCameraConfigFilter
/// Target camera capture mArFrame rates.
/// The target mArFrame rate represents the maximum or desired mArFrame rate. Actual
/// camera capture mArFrame rates can be lower than the target mArFrame rate under low
/// light conditions in order to accommodate longer exposure times.
AR_DEFINE_ENUM(ArCameraConfigTargetFps){
    /// Target 30fps camera capture mArFrame rate.
    ///
    ...
    /// Target 60fps camera capture mArFrame rate.
    ///
...
/// automatically when the ARCore mArSession is paused.
///
/// @param[in]  session            The ARCore mArSession
/// @param[in]  config             The config object to query
...
/// @ingroup ArSession
/// Releases resources used by an ARCore mArSession.
...
/// @ingroup ArSession
/// Configures the mArSession and verifies that the enabled features in the
/// specified mArSession config are supported with the currently set camera config.
///
/// Should be called after @c ::ArSession_setCameraConfig to verify that all
/// requested mArSession config features are supported. Features not supported with
/// the current camera config will otherwise be silently disabled when the
/// mArSession is resumed by calling @c ::ArSession_resume.
///
...
///
/// @param[in] session The ARCore mArSession.
/// @param[in] config The new configuration setting for the mArSession.
///
/// @return @c #AR_SUCCESS or any of:
/// - @c #AR_ERROR_FATAL
/// - @c #AR_ERROR_UNSUPPORTED_CONFIGURATION if the requested mArSession config is
///   not supported.
...
/// @c ::ArSession_configure. Note: if the mArSession was not explicitly
...
/// return @c #AR_ERROR_ILLEGAL_STATE and the mArSession will not resume.
///
/// @return @c #AR_SUCCESS or any of:
/// - @c #AR_ERROR_FATAL
...
/// @ingroup ArSession
/// Pause the current mArSession. This function will stop the camera feed and
/// release resources. The mArSession can be restarted again by calling
/// @c ::ArSession_resume.
///
/// Typically this should be called from <a
...
/// camera mArFrame, updating the location of the device, updating the location of
/// tracking anchors, updating detected planes, etc.
///
/// This call may cause off-screen OpenGL activity. Because of this, to avoid
/// unnecessary mArFrame buffer flushes and reloads, this call should not be made
/// in the middle of rendering a mArFrame or offscreen buffer.
///
...
/// some applications it may be important to know if a new mArFrame was actually
/// obtained (for example, to avoid redrawing if the camera did not produce a
/// new mArFrame). To do that, compare the current mArFrame's timestamp, obtained via
/// @c ::ArFrame_getTimestamp, with the previously recorded mArFrame timestamp. If
/// they are different, this is a new mArFrame.
///
/// During startup the camera system may not produce actual images
/// immediately. In this common case, a mArFrame with timestamp = 0 will be
/// returned.
///
/// @param[in]    session   The ARCore mArSession
/// @param[inout] out_frame The Frame object to populate with the updated world
///     state.  This mArFrame must have been previously created using
...
/// A mArSession can be resolving up to 40 Cloud Anchors at a given time.
///
...
///
/// Hosting requires an active mArSession for which the @c ::ArTrackingState
...
/// configs supported by the configured mArSession and camera.
///
...
/// device capabilities.
///
/// When the mArSession camera is a back-facing camera:
...
///
/// When the mArSession camera is front-facing (selfie) camera, the list will
...
/// - In ARCore SDK 1.7.0 and 1.8.0, when the mArSession camera was a front-facing
...
/// called while the mArSession is paused. The provided @c ::ArCameraConfig must
/// be one of the configs returned by
/// @c ::ArSession_getSupportedCameraConfigsWithFilter.
///
/// The camera config will be applied once the mArSession is resumed.
/// All previously acquired mArFrame images must be released with
...
/// a new camera config.
///
/// Changing the camera config for an existing mArSession may affect which ARCore
/// features can be used. Unsupported mArSession features are silently disabled
/// when the mArSession is resumed. Call @c ::ArSession_configure after setting a
/// camera config to verify that all configured mArSession features are supported
/// with the new camera config.
///
/// Changing the current mArSession's camera config to one that uses a different
/// camera will cause all internal mArSession states to be reset when the mArSession
...
/// @ingroup ArSession
/// Returns the @c ::ArEarth object for the mArSession. This object is long-lived;
/// it should be used for the entire duration of the ARCore mArSession or until an
...
///     or @c NULL if the mArSession's config @c ::ArGeospatialMode is set to @c
///     #AR_GEOSPATIAL_MODE_DISABLED.
...
///
/// Restrictions:
/// - Can only be called while the mArSession is paused. Playback of the MP4
///   dataset file will start once the mArSession is resumed.
/// - The MP4 dataset file must use the same camera facing direction as is
///   configured in the mArSession.
/// - Due to the way mArSession data is processed, ARCore APIs may sometimes
...
///   pausing the mArSession (by calling @c ::ArSession_pause) will suspend
///   processing of all camera image frames and any other recorded sensor data
///   in the dataset. Camera image frames and sensor mArFrame data that is
///   discarded in this way will not be reprocessed when the mArSession is again
///   resumed (by calling @c ::ArSession_resume). AR tracking for the mArSession
...
///
/// @return @c #AR_SUCCESS or any of:
/// - @c #AR_ERROR_SESSION_NOT_PAUSED if called when mArSession is not paused.
...
AR_DEFINE_ENUM(ArPlaybackStatus){
    /// The mArSession is not playing back an MP4 dataset file.
    AR_PLAYBACK_NONE = 0,
...
/// again during the next mArFrame.
/// - #AR_ERROR_INVALID_ARGUMENT when any argument is invalid, e.g. null
/// - #AR_ERROR_DEADLINE_EXCEEDED when the mArFrame is not the current mArFrame
...
/// latest mArFrame. This is an OpenGL camera pose with +X pointing right, +Y
...
///
/// @param[in]    session  The ARCore mArSession
/// @param[in]    camera   The mArSession's camera (retrieved from any mArFrame).
...
/// content onto the latest mArFrame. This is an OpenGL camera pose with +X
...
/// * @c ::ArFrame_getAndroidSensorPose for the pose of the android sensor
/// mArFrame. It will differ in both orientation and location.
...
/// @ingroup ArCamera
/// Returns the view matrix for the camera for this mArFrame. This matrix performs
/// the inverse transform as the pose provided by
/// @c ::ArCamera_getDisplayOrientedPose.
///
/// @param[in]    session           The ARCore mArSession
/// @param[in]    camera            The mArSession's camera.
...
/// The intrinsics may change per mArFrame, so this should be called
/// on each mArFrame to get the intrinsics for the current mArFrame.
///
/// @param[in]    session                The ARCore mArSession
/// @param[in]    camera                 The mArSession's camera.
...
/// stream. The intrinsics may change per mArFrame, so this should be called
/// on each mArFrame to get the intrinsics for the current mArFrame.
///
/// @param[in]    session                The ARCore mArSession
/// @param[in]    camera                 The mArSession's camera.
...
/// be used to detect dropped frames or measure the camera mArFrame rate. The time
...
/// href="https://developer.android.com/guide/topics/sensors/sensors_overview#sensors-coords">Android
/// Sensor Coordinate System</a> in the world coordinate space for this mArFrame.
...
///
/// @param[in]    session  The ARCore mArSession
/// @param[in]    frame    The current mArFrame.
/// @param[inout] out_pose An already-allocated @c ::ArPose object into which
///     the pose will be stored.
...
/// function is called on an older mArFrame, a log message will be printed and
...
///
/// Note: If called on an old mArFrame (not the latest produced by
...
/// @c #AR_INSTANT_PLACEMENT_MODE_LOCAL_Y_UP in the ARCore mArSession
/// configuration, the ARCore mArSession tracking state is @c
...
/// @return @c #AR_SUCCESS or any of:
/// - @c #AR_ERROR_DEADLINE_EXCEEDED if @p mArFrame is not the latest mArFrame from
...
/// @ingroup ArFrame
/// Returns the camera object for the mArSession. Note that this Camera instance is
/// long-lived so the same instance is returned regardless of the mArFrame object
...
/// for
///   the given mArFrame. Note: this commonly happens for few frames right
...
/// @ingroup ArFrame
/// Returns the CPU image for the current mArFrame.
...
/// - @c #AR_ERROR_DEADLINE_EXCEEDED - the input mArFrame is not the current mArFrame.
...
///   mArFrame was not found within a bounded amount of time, or the camera failed
...
/// attached to a later mArFrame in order.
///
/// Note, currently playback continues internally while the mArSession is paused.
/// Track data from frames that were processed while the mArSession was
...
/// @ingroup ArTrackData
/// Retrieves the timestamp in nanoseconds of the mArFrame the given
/// @c ::ArTrackData was recorded on. This timestamp is equal to the result of
/// @c ::ArFrame_getTimestamp on the mArFrame during which the track data was
/// written.
///
/// @param[in]    session            The ARCore mArSession
...
/// @ingroup ArFrame
/// Attempts to acquire a depth image that corresponds to the current mArFrame.
///
...
///
/// If an up-to-date depth image isn't ready for the current mArFrame, the most
/// recent depth image available from an earlier mArFrame will be returned instead.
...
///     with @c nullptr.
/// @return @c #AR_SUCCESS or any of:
/// - @c #AR_ERROR_INVALID_ARGUMENT if the mArSession, mArFrame, or depth image
...
///   observed.
/// - @c #AR_ERROR_NOT_TRACKING The mArSession is not in the
...
/// image data from a previous mArFrame to account for device movement since the
...
/// @ingroup ArFrame
/// Returns the OpenGL ES camera texture name (ID) associated with this mArFrame.
...
/// Point Cloud data mArFrame to mArFrame, consider copying just the data points your
...
///
/// Each point has a unique identifier (within a mArSession) that is persistent
...
AR_DEFINE_ENUM(ArImageFormat){
    /// Invalid image format. Produced by ARCore when an invalid mArSession/image
...
/// - @c #AR_ERROR_SESSION_PAUSED if the mArSession was paused
...
/// single mArFrame, create an anchor using @c ::ArHitResult_acquireNewAnchor to
...
/// @ingroup ArEarth
/// Returns the Earth-relative virtual camera pose for the latest mArFrame.
///
...
/// an east-up-south coordinate mArFrame. An identity rotation will have the anchor
...
///   range, or if either @p mArSession, @p earth, or @p eus_quaternion_4 is @c
///   NULL.
...
/// at a time, but a mArSession cannot be tracking more than 40 Terrain Anchors at
...
///
/// @return @c #AR_SUCCESS or any of:
/// - @c #AR_ERROR_INVALID_ARGUMENT - if either @p mArSession, @p image_metadata or
